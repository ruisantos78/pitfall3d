...
export function createCrocodileModel(voxelSize = 0.32) {
  ...
  // A. Skull & Body (Safe Stepping Platform behind and between the eyes: Z = -10 to 2)
  for (let z = -10; z <= 2; z++) {
    for (let x = -4; x <= 4; x++) {
      ...
      if (isRim) {
        ...
        const isCenterChevron = Math.abs(x) <= 1 && (z === -2 || z === -4 || z === -6 || z === 0);
        if (isCenterChevron) {
  ...
  for (let z = -11; z <= 13; z++) {
    for (let x = -5; x <= 5; x++) {
      const isPerimeter = Math.abs(x) === 5 || z === -11 || z === 13;
      if (isPerimeter) {
